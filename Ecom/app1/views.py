...
def product_detail(request, pid):
    ...
    users_with_reviews = UserDetails.objects.filter(user_productreview_product=product)
    try:
...
def filter_product(request):    
    try:
            ...
            if len(categories) > 0:
                products = products.filter(category_cid_in=categories).distinct()
...
def cart_view(request):
    ...
    available_coupons = Coupon.objects.filter(active=True, active_date_lte=current_date, expiry_date_gte=current_date)
